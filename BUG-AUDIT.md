...
### Tick-crash family sweep
After the fix, both repos were swept for the same family (in-place mutation of struct memory backing a live details view, and re-entrant Slate refresh/expansion during callbacks). **The details-view family is isolated to the widget fixed above — no other site repeats it.** Checked and cleared:
- `GorgeousAnimTransitionRuleCustomization` (renders the `AnimRules` elements *inside* the crashing struct) — correct pattern: `PropertyHandle->SetValue()` + deferred `RequestForceRefresh()`; never touches raw struct memory. **Clean.**
- `GorgeousDataSchemaMappingEditorToolkit` — `CreateDetailView` + `SetObject(asset)` once over a persistent UObject. Standard asset-editor pattern. **Clean.**
- `GorgeousObjectVariableBrowserWindow` tree — `SetItemExpansion` only from explicit button handlers, not re-entrantly inside `OnGetChildren`. **Clean.**
- `GorgeousItemEquipAttachmentKeyCustomization` (customizes the key struct itself) — no risky refresh/raw-data patterns. **Clean.**
- Residual in-place `GetStructMemory()->RotationOffset = …` writes in the fixed widget — safe: `RotationOffset` is a hidden `UPROPERTY()` (no `EditAnywhere`) fixed-size POD, so no detail node references it.

Related-but-milder sibling found: finding **G** below (`GorgeousInsightDebugPanel` `RebuildList()` from click handlers) — now switched to the deferred `RequestListRefresh()`.
...
| G | ✅ RESOLVED | `GorgeousCoreRuntime/.../InsightMatrix/Slate/GorgeousInsightDebugPanel.cpp:1589 & :1637` | Per-row button handlers (`OnRunTestClicked`, `OnRunQueuedTestsClicked`) called `TestsListView->RebuildList()`, regenerating all rows **synchronously** from inside a row button's click. Likely not an actual crash (Slate's event path holds a `TSharedRef` to the clicked button during the event), but needlessly re-entrant and inconsistent with the deferred `RequestListRefresh()` used everywhere else in the file. **Fixed** (commit `7f3fc44`): both switched to `RequestListRefresh()`. |
