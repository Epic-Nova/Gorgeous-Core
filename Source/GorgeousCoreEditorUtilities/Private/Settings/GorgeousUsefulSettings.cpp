...
bool UGorgeousUsefulSettings::WriteIniSettingDirect(const FString& IniFilePath, const FString& Section, const FString& Key, const FString& Value, bool bReloadIntoGConfig)
{
	...
	else
	{
		...
		int32 SectionIndex = FileContents.Find(SectionHeader, ESearchCase::IgnoreCase);
...
void UGorgeousUsefulSettings::SyncFromEditorSettings()
{
	...
	if (GConfig)
	{
		...
		FString LogStreamingValue;
		if (GConfig->GetString(CoreLogSection, LogStreamingKey, LogStreamingValue, GEngineIni))
		{
			bVerboseLogStreaming = IsVerboseLogSetting(LogStreamingValue);
		}

		FString LogGarbageValue;
		if (GConfig->GetString(CoreLogSection, LogGarbageKey, LogGarbageValue, GEngineIni))
		{
...
void UGorgeousUsefulSettings::ApplyToEditorSettings()
{
